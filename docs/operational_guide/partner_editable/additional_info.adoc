...
//TODO Vinod, What do we need to sign in to first? 

. Go to the boot node. Look under the **Resources** tab of the boot-node stack. The node ID is listed in **Outputs** of the main stack.
. Start the boot node.
...
//TODO Vinod, I combined two sections here, as seemed logical. Does this flow okay as edited? 
